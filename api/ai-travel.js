...
async function logUsage({ userId, userEmail, model, inputTokens, outputTokens, destination, success, error }) {
  try {
    // Rough cost estimate for GPT-4o. Web search may add a small per-call fee
    // on top of this — check current OpenAI pricing for exact figures.
...
async function getCachedChecklist(originCountry, destinationCountry) {
  if (!originCountry || !destinationCountry) return null;
  const cutoff = new Date(Date.now() - CACHE_TTL_DAYS * 24 * 60 * 60 * 1000).toISOString();
  try {
    const res = await fetch(
      `${process.env.SUPABASE_URL}/rest/v1/travel_route_cache?origin_country=eq.${encodeURIComponent(normalizeCountry(originCountry))}&destination_country=eq.${encodeURIComponent(normalizeCountry(destinationCountry))}&created_at=gte.${cutoff}&select=checklist_json&order=created_at.desc&limit=1`,
...
async function saveToCache(originCountry, destinationCountry, checklistItems) {
...
export default async function handler(req, res) {
  ...
  const { messages, userId, userEmail, destination, originCountry, destinationCountry } = req.body;
  ...
  const cached = await getCachedChecklist(originCountry, destinationCountry);
  if (cached) {
    console.log(`Cache hit for ${originCountry} → ${destinationCountry}`);
  ...
  try {
    ...
    checklistItems = await verifyWithClaude(checklistItems);

    // Save to cache so the next request for this route is free
    await saveToCache(originCountry, destinationCountry, checklistItems);
