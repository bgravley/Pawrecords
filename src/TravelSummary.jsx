  ...
  return <main style={{ minHeight: '100vh', background: C.white, color: C.text, fontFamily: "'Lora', serif", paddingBottom: 40 }}>
    ...
    <div style={{ maxWidth: 760, margin: 'auto', padding: 18 }}>
      ...
      <SummarySection title="Travel references">
        ...
        {officialFees.map(item => <p key={item.title}><strong>{item.title}: {feeLabel(item)}</strong>{item.fee_basis ? ` · ${item.fee_basis}` : ''}<br /><span style={{ color: C.sage, fontSize: 12 }}>Checked {formatTravelDate(item.fee_last_checked_at || item.researched_at)} · {safeExternalUrl(item.source_url) ? <a href={safeExternalUrl(item.source_url)} target="_blank" rel="noreferrer" style={{ color: C.green }}>Official source</a> : 'Source unavailable'}. Additional fees may apply.</span></p>)}
      ...
      <SummarySection title="Requirements"><ul>{summary.checklist.length ? summary.checklist.map((item, index) => <li key={index}><strong>{item.title}</strong> · {(item.readiness_status || (item.is_completed ? 'complete' : 'missing')).replaceAll('_', ' ')}{safeExternalUrl(item.source_url) ? <> · <a href={safeExternalUrl(item.source_url)} target="_blank" rel="noreferrer" style={{ color: C.green }}>{item.source_authority || 'Official source'}</a></> : ' · Source needed'}</li>) : <li>No requirements added yet.</li>}</ul></SummarySection>
