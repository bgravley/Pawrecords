...
export default function TravelSummary({ token }) {
  ...
  const print = () => {
    ...
    if (!popup) return;
    popup.document.write(buildTravelSummaryHtml({ ...summary, shareUrl }));
  ...
  return <main style={{ minHeight: '100vh', background: C.white, color: C.text, fontFamily: "'Lora', serif", paddingBottom: 40 }}>
    ...
    <div style={{ maxWidth: 760, margin: 'auto', padding: '18px' }}>
      ...
      {[['Trip details', <div key="trip">Departure: {formatTravelDate(summary.trip.departure_date)}<br />Overall travel: {summary.trip.transportation_type || 'air'}</div>], ['Flights & itinerary', <div key="legs">{summary.legs.map((leg, index) => <p key={index}><strong>{leg.origin_city}{leg.origin_airport_code ? ` (${leg.origin_airport_code})` : ''} → {leg.destination_city}{leg.destination_airport_code ? ` (${leg.destination_airport_code})` : ''}</strong><br /><span style={{ color: C.sage }}>{formatTravelDate(leg.departure_date)} · {leg.airline || leg.transportation_type} {leg.flight_number || ''}</span></p>)}</div>], ['Traveling pets', <div key="pets">{pets.map(pet => <div key={pet.id} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: `1px solid ${C.cream}` }}>{pet.photo_url ? <img src={pet.photo_url} alt={pet.name} style={{ width: 56, height: 56, borderRadius: '50%', objectFit: 'cover' }} /> : <div style={{ width: 56, height: 56, borderRadius: '50%', background: C.cream, display: 'grid', placeItems: 'center' }}>🐾</div>}<div><strong>{pet.name}</strong><div style={{ color: C.sage, fontSize: 13 }}>{pet.breed || pet.species || 'Pet'} · {pet.travel_arrangement_label}<br />Microchip: {pet.microchip || 'Not added'}<br />{pet.pet_type === 'service_animal' || pet.is_service_animal ? 'Service animal' : pet.pet_type === 'esa' || pet.is_esa ? 'Emotional support animal' : 'Pet'}{pet.emergency_contact || pet.emergency_phone ? <><br />Emergency: {[pet.emergency_contact, pet.emergency_phone].filter(Boolean).join(' · ')}</> : null}</div></div></div>)}</div>], ['Documents, permits & labs', <ul key="docs">{summary.documents.length ? summary.documents.map((doc, index) => <li key={index}>{doc.name}{doc.doc_date ? ` · ${formatTravelDate(doc.doc_date)}` : ''}</li>) : <li>No travel documents added yet.</li>}</ul>], ['Requirements', <ul key="requirements">{summary.checklist.length ? summary.checklist.map((item, index) => <li key={index}><strong>{item.title}</strong> · {(item.readiness_status || (item.is_completed ? 'complete' : 'missing')).replaceAll('_', ' ')}{item.source_url ? <> · <a href={item.source_url} target="_blank" rel="noreferrer" style={{ color: C.green }}>{item.source_authority || 'Official source'}</a></> : ' · Source needed'}</li>) : <li>No requirements added yet.</li>}</ul>], ['Secure QR', <div key="qr" style={{ textAlign: 'center' }}><QrCode value={shareUrl} /><p style={{ color: C.sage, fontSize: 12 }}>This private link expires {formatTravelDate(summary.trip.travel_share_expires_at)}.</p></div>]].map(([title, body]) => <section key={title} style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 14 }}><h2 style={{ fontFamily: "'Playfair Display', serif", color: C.green, fontSize: 20, margin: '0 0 12px' }}>{title}</h2>{body}</section>)}
      <p style={{ color: C.sage, fontSize: 12, lineHeight: 1.6 }}>Country requirements should always be verified with the responsible government authority. Airline and airport sources are used only for their own policies and logistics.</p>
    </div>
  </main>;
}
