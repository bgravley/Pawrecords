...
export function buildTravelSummaryHtml({ trip, legs = [], pets = [], documents = [], checklist = [], shareUrl = '' }) {
  ...
  ${section('Itinerary', itinerary)}${section('Traveling pets', petCards || '<p>No pets selected.</p>')}${section('Travel references', travelReferences)}${section('Health documents, permits & labs', documentSection)}${section('Requirements status', requirementRows ? `<table><tbody>${requirementRows}</tbody></table>` : '<p>No requirements added yet.</p>')}${shareUrl ? section('Secure shared view', `<p class="notice">This private link expires automatically. Only share it with people helping with this trip.<br><a href="${escapeHtml(shareUrl)}">${escapeHtml(shareUrl)}</a></p>`) : ''}<section class="notice"><span class="brand">YourPetPass</span> keeps your pet's travel details together. Always verify country rules and fees with the responsible government authority and airline or airport logistics with their official sources.</section></main></body></html>`;
}
