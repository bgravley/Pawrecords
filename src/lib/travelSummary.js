...
export function buildTravelSummaryHtml({ trip, legs = [], pets = [], documents = [], checklist = [], shareUrl = '' }) {
  ...
  const requirementRows = checklist.map(item => `<tr><th>${escapeHtml(item.title)}</th><td>${escapeHtml(item.readiness_status || (item.is_completed ? 'complete' : 'missing')).replace(/_/g, ' ')}</td><td>${item.source_url ? `<a href="${escapeHtml(item.source_url)}">${escapeHtml(item.source_authority || 'Official source')}</a>` : 'Source needed'}</td></tr>`).join('');
  ...
  ${section('Itinerary', itinerary)}${section('Traveling pets', petCards || '<p>No pets selected.</p>')}${section('Health documents, permits & labs', documentSection)}${section('Requirements status', requirementRows ? `<table><tbody>${requirementRows}</tbody></table>` : '<p>No requirements added yet.</p>')}${shareUrl ? section('Secure shared view', `<p class="notice">This private link expires automatically. Only share it with people helping with this trip.<br><a href="${escapeHtml(shareUrl)}">${escapeHtml(shareUrl)}</a></p>`) : ''}<section class="notice"><span class="brand">YourPetPass</span> keeps your pet's travel details together. Always verify country rules with the responsible government authority and airline or airport logistics with their official sources.</section></main></body></html>`;
}
