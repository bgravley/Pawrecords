...
// YourPetPass v3 — Supabase data + Stripe payments + free/premium gating
...
const C = {
  ...
  accent:"#2D7D6F",accentDim:"#2D7D6F14",warn:"#E8A838",warnDim:"#E8A83814",
  danger:"#C4714A",dangerDim:"#C4714A14",blue:"#2D7D6F",blueDim:"#2D7D6F14",
  gold:"#E8A838",purple:"#5A4535",text:"#2C2017",sub:"#5A4535",muted:"#8B7355",navH:64,
};
...
const exportICS=(dogName,vacName,due)=>{
  ...
  const ics=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//YourPetPass//EN","BEGIN:VEVENT",
    `DTSTART:${ts}`,`DTEND:${ts}`,`SUMMARY:${dogName} — ${vacName} due`,
    `DESCRIPTION:${vacName} vaccination due for ${dogName}. Managed in YourPetPass.`,
...
<div class="gen">Generated ${new Date().toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})} · YourPetPass</div>
<h2>Profile</h2><table>
<tr><td><b>Name</b></td><td>${dog.name}</td></tr>
<tr><td><b>Breed</b></td><td>${dog.breed||"—"}</td></tr>
<tr><td><b>Born</b></td><td>${fmt(dog.dob)}</td></tr>
<tr><td><b>Weight</b></td><td>${dog.weight?dog.weight+" lbs":"—"}</td></tr>
<tr><td><b>Microchip</b></td><td>${dog.microchip||"—"}</td></tr>
${dog.emergency_contact?`<tr><td><b>Emergency</b></td><td>${dog.emergency_contact} ${dog.emergency_phone||""}</td></tr>`:""}
</table>
${al.length?`<h2>⚠️ Allergies</h2><table><tr><th>Allergen</th><th>Reaction</th><th>Severity</th></tr>
${al.map(a=>`<tr><td><b>${a.allergen}</b></td><td>${a.reaction}</td><td>${a.severity.toUpperCase()}</td></tr>`).join("")}</table>`:""}
<h2>Vaccinations</h2>${v.length?`<table><tr><th>Vaccine</th><th>Type</th><th>Given</th><th>Next Due</th><th>Vet</th></tr>
${v.map(x=>`<tr><td><b>${x.name}</b></td><td><span class="${x.type==="core"?"core":"opt"}">${x.type.toUpperCase()}</span></td><td>${fmt(x.date_given)}</td><td>${fmt(x.next_due)}</td><td>${x.vet_name||"—"}</td></tr>`).join("")}</table>`:"<p>No vaccinations recorded.</p>"}
${m.length?`<h2>Medications</h2><table><tr><th>Medication</th><th>Dosage</th><th>Frequency</th><th>Status</th></tr>
${m.map(x=>`<tr><td><b>${x.name}</b></td><td>${x.dosage}</td><td>${x.frequency}</td><td>${x.active?"Active":"Completed"}</td></tr>`).join("")}</table>`:""}
${vs.length?`<h2>Vet Visits</h2><table><tr><th>Date</th><th>Vet/Clinic</th><th>Reason</th><th>Diagnosis</th></tr>
${vs.map(x=>`<tr><td>${fmt(x.visit_date)}</td><td>${x.vet_name||"—"}<br><small>${x.clinic||""}</small></td><td>${x.reason}</td><td>${x.diagnosis||"—"}</td></tr>`).join("")}</table>`:""}
  ...
  render(){
    if(this.state.err)return(
      <div style={{minHeight:"100vh",background:"#FAF6F0",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{background:"#FFFFFF",border:"1px solid #C4714A44",borderRadius:16,padding:28,maxWidth:480,width:"100%"}}>
          <div style={{color:"#C4714A",fontWeight:700,fontSize:18,marginBottom:12}}>⚠ Something crashed</div>
          <div style={{fontFamily:"monospace",fontSize:12,color:"#5A4535",background:"#FFFFFF",borderRadius:10,padding:14,
          ...
          <button onClick={()=>this.setState({err:null,info:null})}
            style={{background:"#2D7D6F",color:"#FAF6F0",borderRadius:10,padding:"10px 20px",fontWeight:600,fontSize:14,border:"none",cursor:"pointer"}}>
            Try to Recover
          </button>
        </div>
      </div>
...
const Card=({children,style:s,onClick})=>(
  <div onClick={onClick} style={{background:"#FFFFFF",border:"1px solid #E8DDD0",borderRadius:16,padding:18,boxShadow:"0 2px 12px rgba(44,32,23,0.06)",...s,cursor:onClick?"pointer":"default",transition:"border-color .2s"}}
    onMouseEnter={e=>onClick&&(e.currentTarget.style.borderColor="#2D7D6F55")}
    onMouseLeave={e=>onClick&&(e.currentTarget.style.borderColor="#E8DDD0")}>{children}</div>
);
...
const Modal=({title,onClose,children})=>(
  <div style={{position:"fixed",inset:0,background:"#000000bb",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:"#FFFFFF",border:"1px solid #E8DDD0",borderRadius:20,width:"100%",maxWidth:500,maxHeight:"92vh",overflow:"auto",padding:24,boxShadow:"0 8px 40px rgba(44,32,23,0.15)"}} className="fade">
...
const PremiumLock=({onUpgrade,label="Premium Feature"})=>(
  <div style={{background:"#E8A83810",border:"1px solid #E8A83844",borderRadius:12,padding:16,textAlign:"center"}}>
    ...
    <Btn onClick={onUpgrade} style={{margin:"0 auto",background:"#E8A838",color:"#FAF6F0"}}>
      <Ic n="crown" s={14} c="#FAF6F0"/> Upgrade Now
    </Btn>
  </div>
);
  ...
  const checkout=async(priceKey,mode)=>{
    ...
    try{
      const{data,error:fnErr}=await supabase.functions.invoke("create-checkout",{
        body:{priceId:PRICES[priceKey],userId,userEmail,mode}
      });
  ...
  return(
    <Modal title="Upgrade to Premium" onClose={onClose}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {plans.map(plan=>(
          <div key={plan.key} style={{position:"relative",border:`2px solid ${plan.popular?"#2D7D6F":"#E8DDD0"}`,borderRadius:14,padding:18,
            background:plan.popular?"#2D7D6F08":"transparent"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{color:"#5A4535",fontSize:13,marginTop:2}}>{plan.desc}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontFamily:"'Lora',serif",fontSize:26,color:plan.color,fontWeight:700}}>{plan.price}</div>
                <div style={{color:"#5A4535",fontSize:12}}>{plan.period}</div>
              </div>
            </div>
            <Btn full onClick={()=>checkout(plan.key,plan.mode)} disabled={!!loading}
              style={{marginTop:12,background:plan.color,color:plan.key==="monthly"?"#fff":"#FAF6F0",justifyContent:"center"}}>
              {loading===plan.key?"Processing...":`Get ${plan.label}`}
            </Btn>
          </div>
        ))}
        <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",color:"#5A4535",fontSize:12,marginTop:4}}>
          <Ic n="lock" s={12} c="#5A4535"/> Secure payment via Stripe
        </div>
      </div>
    </Modal>
  );
      ...
      {items.map(item=>{
        ...
        return(<button key={item.id} onClick={()=>setTab(item.id)}
          style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
            gap:3,background:"none",color:active?"#2D7D6F":"#5A4535",fontSize:11,fontWeight:active?600:400,
          ...
          {item.id==="vaccines"&&alerts>0&&(
            <span style={{position:"absolute",top:8,left:"calc(50% + 6px)",background:"#C4714A",color:"#fff",
...
const DogForm=({dog,userId,onSave,onClose})=>{
  const[f,setF]=useState(dog?{
    name:dog.name,breed:dog.breed||"",dob:dog.dob||"",weight:dog.weight||"",
    gender:dog.gender||"male",neutered:dog.neutered||false,microchip:dog.microchip||"",
    color:dog.color||"",emergencyContact:dog.emergency_contact||"",
    emergencyPhone:dog.emergency_phone||"",notes:dog.notes||"",photo:dog.photo_url||""
  }:{name:"",breed:"",dob:"",weight:"",gender:"male",neutered:false,microchip:"",color:"",emergencyContact:"",emergencyPhone:"",notes:"",photo:""});
  ...
  const save=async()=>{
    ...
    const payload={name:f.name,breed:f.breed,dob:f.dob||null,weight:f.weight||null,
      gender:f.gender,neutered:f.neutered,microchip:f.microchip,color:f.color,
      emergency_contact:f.emergencyContact,emergency_phone:f.emergencyPhone,notes:f.notes};
    let result;
  ...
  return(<Modal title={dog?"Edit Profile":"Add Pet"} onClose={onClose}>
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",justifyContent:"center"}}>
        <div style={{position:"relative",cursor:"pointer"}} onClick={()=>fr.current.click()}>
          {f.photo?<img src={f.photo} style={{width:80,height:80,borderRadius:"50%",objectFit:"cover",border:"3px solid #2D7D6F"}}/>
      ...
      <div style={{display:"flex",gap:10,marginTop:4}}>
        ...
        <Btn onClick={save} disabled={saving} full>{saving?"Saving...":"Save Pet"}</Btn>
      </div>
    </div>
  </Modal>);
};
...
const VaccineForm=({vacc,dogId,userId,onSave,onClose})=>{
  const[f,setF]=useState(vacc?{
    name:vacc.name,type:vacc.type,dateGiven:vacc.date_given||today(),
    durationMonths:vacc.duration_months||12,nextDue:vacc.next_due||"",
    lotNumber:vacc.lot_number||"",vetName:vacc.vet_name||"",notes:vacc.notes||""
  }:{name:"",type:"core",dateGiven:today(),durationMonths:12,nextDue:"",lotNumber:"",vetName:"",notes:""});
  const[saving,setSaving]=useState(false);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  ...
  const save=async()=>{
    if(!f.name)return;setSaving(true);
    const payload={dogId,name:f.name,type:f.type,dateGiven:f.dateGiven,nextDue:f.nextDue,
      durationMonths:parseInt(f.durationMonths),lotNumber:f.lotNumber,vetName:f.vetName,notes:f.notes};
    let result;
...
const MedForm=({med,dogId,userId,onSave,onClose})=>{
  const[f,setF]=useState(med?{
    ...
    startDate:med.start_date||today(),endDate:med.end_date||"",
    prescribingVet:med.prescribing_vet||"",reason:med.reason||"",notes:med.notes||"",active:med.active
  }:{name:"",dosage:"",frequency:"",startDate:today(),endDate:"",prescribingVet:"",reason:"",notes:"",active:true});
  const[saving,setSaving]=useState(false);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));

  const save=async()=>{
    if(!f.name)return;setSaving(true);
    const payload={dogId,name:f.name,dosage:f.dosage,frequency:f.frequency,startDate:f.startDate,
      endDate:f.endDate||null,prescribingVet:f.prescribingVet,reason:f.reason,notes:f.notes,active:f.active};
    let result;
...
const AllergyForm=({allergy,dogId,userId,onSave,onClose})=>{
  const[f,setF]=useState(allergy?{allergen:allergy.allergen,reaction:allergy.reaction||"",severity:allergy.severity,dateDiscovered:allergy.date_discovered||today(),notes:allergy.notes||""}
    :{allergen:"",reaction:"",severity:"mild",dateDiscovered:today(),notes:""});
  const[saving,setSaving]=useState(false);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
...
const VisitForm=({visit,dogId,userId,onSave,onClose})=>{
  const[f,setF]=useState(visit?{date:visit.visit_date,vetName:visit.vet_name||"",clinic:visit.clinic||"",
    reason:visit.reason,diagnosis:visit.diagnosis||"",treatment:visit.treatment||"",cost:visit.cost||"",notes:visit.notes||""}
    :{date:today(),vetName:"",clinic:"",reason:"",diagnosis:"",treatment:"",cost:"",notes:""});
  const[saving,setSaving]=useState(false);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));

  const save=async()=>{
    if(!f.reason)return;setSaving(true);
    const payload={dogId,date:f.date,vetName:f.vetName,clinic:f.clinic,reason:f.reason,
      diagnosis:f.diagnosis,treatment:f.treatment,cost:f.cost||null,notes:f.notes};
    let result;
...
const ShareModal=({dog,onClose})=>{
  const msgs=[
    `🐾 ${dog.name} is up to date on all vaccinations! Keeping pet records organized with YourPetPass 📋`,
    `💉 Just logged ${dog.name}'s latest vet visit! Healthy pup = happy life. #YourPetPass #DogMom #DogDad`,
  ...
  return(<Modal title={`Share ${dog.name}'s Update`} onClose={onClose}>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {msgs.map((m,i)=><div key={i} onClick={()=>setSel(i)} style={{padding:14,borderRadius:12,
        border:`2px solid ${sel===i?"#2D7D6F":"#E8DDD0"}`,cursor:"pointer",fontSize:14,lineHeight:1.6,
        background:sel===i?"#2D7D6F14":"#FFFFFF"}}>{m}</div>)}
...
const AIScanModal=({dog,userId,onSave,onClose})=>{
  ...
  const saveAll=async()=>{
    setSaving(true);
    try{
      if(include.visit&&(extracted.reason||extracted.visitDate)){
        await db.addVisit(userId,{dogId:dog.id,date:extracted.visitDate||today(),vetName:extracted.vetName||"",
          clinic:extracted.clinicName||"",reason:extracted.reason||"Vet visit (scanned)",
          diagnosis:extracted.diagnosis||"",treatment:extracted.treatment||"",cost:extracted.cost||null,notes:extracted.notes||""});
      }
      if(include.vaccines)for(const v of(extracted.vaccines||[]).filter(v=>v.name)){
        const dur=([...CORE_V,...OPT_V].find(x=>x.name===v.name)?.dur)||12;
        const given=v.dateGiven||extracted.visitDate||today();
        await db.addVaccination(userId,{dogId:dog.id,name:v.name,type:v.type||"optional",dateGiven:given,
          nextDue:v.nextDue||addM(given,dur),lotNumber:v.lotNumber||"",vetName:extracted.vetName||"",
      ...
      if(include.medications)for(const m of(extracted.medications||[]).filter(m=>m.name)){
        await db.addMedication(userId,{dogId:dog.id,name:m.name,dosage:m.dosage||"",frequency:m.frequency||"",
          reason:m.reason||"",startDate:extracted.visitDate||today(),endDate:null,
          prescribingVet:extracted.vetName||"",notes:"",active:true});
      }
      setSaved(true);
      ...
      <div style={{background:"#FFFFFF",border:"1px solid #2D7D6F44",borderRadius:20,width:"100%",maxWidth:520,maxHeight:"94vh",overflow:"auto",padding:24}} className="fade">
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
          <div>
            ...
            <p style={{color:"#5A4535",fontSize:13,marginTop:2}}>Photo any vet record — AI extracts everything</p>
          </div>
          <button onClick={onClose} style={{background:"#FFFFFF",border:"1px solid #E8DDD0",borderRadius:8,padding:"6px 8px",color:"#5A4535"}}><Ic n="x" s={16}/></button>
        </div>
        ...
        {step==="upload"&&<>
          ...
          {!imageData?(<div style={{border:"2px dashed #2D7D6F44",borderRadius:16,padding:40,textAlign:"center",cursor:"pointer",background:"#2D7D6F08"}} onClick={()=>fr.current.click()}>
            ...
            <div>
              <img src={imageData} style={{width:"100%",borderRadius:14,border:"1px solid #E8DDD0",maxHeight:280,objectFit:"contain",background:"#FFFFFF",marginBottom:16}}/>
        ...
        {step==="review"&&extracted&&(<>
          <div style={{background:"#2D7D6F14",border:"1px solid #2D7D6F44",borderRadius:12,padding:14,marginBottom:16,fontSize:13,color:"#2D7D6F"}}>
            ✓ Document analyzed — review and confirm what to save
          </div>
          {[
            {key:"visit",label:"Vet Visit",icon:"stethoscope",content:<div style={{background:"#FFFFFF",borderRadius:10,padding:12,fontSize:13,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              {[["Date",fmt(extracted.visitDate)],["Vet",extracted.vetName||"—"],["Reason",extracted.reason||"—"],["Diagnosis",extracted.diagnosis||"—"]].map(([k,v])=>(
          ...
          {saved?<div style={{textAlign:"center",padding:"16px 0",color:"#2D7D6F",fontWeight:600,fontSize:16}}>✓ All records saved!</div>
            :<div style={{display:"flex",gap:10}}>
...
// ── TABS ──────────────────────────────────────────────────
const OverviewTab=({dog,state,userId,tier,setModal,onUpgrade})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Card>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[["Breed",dog.breed||"—"],["Born",fmt(dog.dob)],["Weight",dog.weight?dog.weight+" lbs":"—"],
          ...
          <div key={k}><div style={{fontSize:11,color:"#5A4535",textTransform:"uppercase",letterSpacing:".05em",marginBottom:2}}>{k}</div>
          <div style={{fontWeight:500,fontSize:14}}>{v}</div></div>))}
      </div>
      {(dog.emergency_contact||dog.emergency_phone)&&(<div style={{marginTop:12,padding:10,background:"#FFFFFF",borderRadius:10,display:"flex",alignItems:"center",gap:8}}>
        <Ic n="phone" s={14} c="#E8A838"/><span style={{fontSize:13}}><b>Emergency:</b> {dog.emergency_contact} {dog.emergency_phone}</span>
      </div>)}
    ...
    {urgent.length>0&&(<Card style={{border:"1px solid #E8A83844",background:"#E8A83814"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,color:"#E8A838",fontWeight:600,fontSize:14}}><Ic n="alert" s={15} c="#E8A838"/> Attention Needed</div>
      {urgent.map(v=>{const st=vSt(v.next_due);return(<div key={v.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #E8DDD044"}}><span style={{fontSize:14}}>{v.name}</span><Badge label={st.label} color={st.c}/></div>);})}
    </Card>)}
    {al.length>0&&(<Card style={{border:"1px solid #C4714A44"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,color:"#C4714A",fontWeight:600,fontSize:14}}><Ic n="alert" s={15} c="#C4714A"/> Known Allergies</div>
      {al.map(a=>(<div key={a.id} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #E8DDD044",fontSize:14}}><span><b>{a.allergen}</b> — {a.reaction}</span><Badge label={a.severity} color={sev(a.severity)}/></div>))}
    </Card>)}
    {meds.length>0&&(<Card><div style={{fontWeight:600,marginBottom:10,fontSize:14,display:"flex",alignItems:"center",gap:8}}><Ic n="pill" s={14} c="#2D7D6F"/> Active Medications</div>
      {meds.map(m=>(<div key={m.id} style={{fontSize:14,padding:"6px 0",borderBottom:"1px solid #E8DDD044"}}><b>{m.name}</b> · {m.dosage} · {m.frequency}</div>))}</Card>)}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      {premium
        ?<Btn full onClick={()=>exportHTML(dog,state)}><Ic n="download" s={14}/> Export PDF</Btn>
        :<Btn full v="secondary" onClick={onUpgrade} style={{opacity:.7}}><Ic n="lock" s={14}/> Export PDF</Btn>}
      <Btn full v="secondary" onClick={()=>setModal("share")}><Ic n="share" s={14}/> Share</Btn>
    </div>
    {!premium&&<div style={{background:"#E8A83810",border:"1px solid #E8A83833",borderRadius:12,padding:12,textAlign:"center",fontSize:13,color:"#E8A838",cursor:"pointer"}} onClick={onUpgrade}>
      <Ic n="crown" s={14} c="#E8A838"/> Upgrade to Premium for PDF export, AI scanning & more
    </div>}
  </div>);
};
...
const VaccinesTab=({dog,state,dispatch,userId,tier,onUpgrade})=>{
  ...
  const delVacc=async(id)=>{
    await db.deleteVaccination(id);
    dispatch({t:"DEL_VACC",id});
  };

  return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <h3 style={{fontFamily:"'Lora',serif",fontSize:20}}>Vaccinations</h3>
    ...
    {vaccines.length===0?<Empty icon="syringe" title="No vaccinations yet" sub="Record your first vaccination to get started." action={<Btn onClick={()=>setModal({type:"add"})}><Ic n="plus" s={14}/> Record Vaccination</Btn>}/>
      :vaccines.map(v=>{const st=vSt(v.next_due);return(<Card key={v.id}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
          <div style={{flex:1}}>
            <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:8,marginBottom:4}}>
              <span style={{fontWeight:600}}>{v.name}</span>
              <Badge label={v.type==="core"?"CORE":"OPTIONAL"} color={v.type==="core"?"#2D7D6F":"#2D7D6F"}/>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,fontSize:12,color:"#5A4535",marginTop:4}}>
              <span>Given: {fmt(v.date_given)}</span><span>Vet: {v.vet_name||"—"}</span>
            </div>
          </div>
    ...
    {premium
      ?<Card style={{borderStyle:"dashed",opacity:.8}}>
        <div style={{fontSize:11,fontWeight:700,color:"#5A4535",textTransform:"uppercase",letterSpacing:".06em",marginBottom:10}}>Recommended Schedule</div>
        {all.map(rv=>{const rec=vaccines.find(v=>v.name===rv.name);const isCore=CORE_V.includes(rv);return(
          <div key={rv.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #E8DDD033"}}>
            <div><div style={{fontSize:13,fontWeight:500}}>{rv.name}</div><div style={{fontSize:11,color:"#8B7355",marginTop:2}}>{rv.note}</div></div>
...
const HealthTab=({dog,state,dispatch,userId,tier,onUpgrade})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {al.length===0?<Card style={{borderStyle:"dashed"}}><div style={{color:"#5A4535",fontSize:14,textAlign:"center",padding:"12px 0"}}>No allergies recorded. <span style={{color:"#2D7D6F",cursor:"pointer"}} onClick={()=>setModal({type:"addAlrg"})}>Add one</span></div></Card>
      :al.map(a=>(<Card key={a.id} style={{border:`1px solid ${sev(a.severity)}44`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <div style={{fontSize:13,color:"#5A4535"}}>Reaction: {a.reaction}</div></div>
          <div style={{display:"flex",gap:5}}>
    ...
    {meds.filter(m=>m.active).map(m=>(<Card key={m.id} style={{borderColor:"#2D7D6F44"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontWeight:600,marginBottom:4}}>{m.name} <span style={{background:"#2D7D6F14",color:"#2D7D6F",fontSize:11,padding:"2px 8px",borderRadius:20}}>ACTIVE</span></div>
        <div style={{fontSize:13,color:"#5A4535"}}>{m.dosage} · {m.frequency}</div></div>
        <div style={{display:"flex",gap:5}}>
...
const RecordsTab=({dog,state,dispatch,userId})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <h3 style={{fontFamily:"'Lora',serif",fontSize:20}}>Vet Visits</h3>
      ...
      :visits.map(v=>(<Card key={v.id}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:"#5A4535",marginTop:2}}>{fmt(v.visit_date)}{v.vet_name?` · ${v.vet_name}`:""}{v.clinic?` @ ${v.clinic}`:""}</div>
            {v.diagnosis&&<div style={{fontSize:13,marginTop:6}}><span style={{color:"#8B7355"}}>Dx: </span>{v.diagnosis}</div>}
            {v.treatment&&<div style={{fontSize:13}}><span style={{color:"#8B7355"}}>Tx: </span>{v.treatment}</div>}
            {v.cost&&<div style={{fontSize:13,color:"#2D7D6F",marginTop:4}}>${v.cost}</div>}
          </div>
...
const MoreTab=({dog,state,dispatch,userId,tier,onUpgrade})=>{
  ...
  const docs=state.documents.filter(d=>d.dog_id===dog.id).sort((a,b)=>b.doc_date?.localeCompare(a.doc_date));
  ...
  if(section==="weight")return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>{backBtn}<h3 style={{fontFamily:"'Lora',serif",fontSize:20,flex:1}}>Weight History</h3>
      {premium&&<Btn sm onClick={()=>setModal("addWeight")}><Ic n="plus" s={14}/> Log</Btn>}</div>
    {premium?<>
      {weights.length>=2&&(<Card><ResponsiveContainer width="100%" height={180}>
        <LineChart data={weights.map(w=>({date:w.log_date.slice(5),weight:w.weight_lbs}))}>
          <CartesianGrid strokeDasharray="3 3" stroke="#E8DDD0"/>
          <XAxis dataKey="date" stroke="#5A4535" tick={{fontSize:11}}/>
          <YAxis stroke="#5A4535" tick={{fontSize:11}} domain={["auto","auto"]}/>
          <Tooltip contentStyle={{background:"#FFFFFF",border:"1px solid #E8DDD0",borderRadius:10,color:"#2C2017",fontSize:13}} formatter={v=>[v+" lbs","Weight"]}/>
          <Line type="monotone" dataKey="weight" stroke="#2D7D6F" strokeWidth={2} dot={{r:3,fill:"#2D7D6F"}}/>
        </LineChart>
      ...
      {weights.length===0?<Empty icon="weight" title="No weight records" sub="Log weight at each vet visit to track trends." action={<Btn onClick={()=>setModal("addWeight")}><Ic n="plus" s={14}/> Log Weight</Btn>}/>
        :weights.slice().reverse().map(w=>(<Card key={w.id}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><span style={{fontFamily:"'Lora',serif",fontSize:22,fontWeight:600}}>{w.weight_lbs}<span style={{fontSize:14,color:"#5A4535"}}> lbs</span></span>
            <div style={{fontSize:12,color:"#5A4535",marginTop:2}}>{fmt(w.log_date)}{w.notes?` · ${w.notes}`:""}</div></div>
            <button onClick={async()=>{await db.deleteWeight(w.id);dispatch({t:"DEL_WT",id:w.id});}} style={{background:"#C4714A14",border:"1px solid #C4714A44",borderRadius:8,padding:"5px 8px",color:"#C4714A"}}><Ic n="trash" s={13}/></button>
          </div>
        </Card>))}
      {modal==="addWeight"&&<Modal title="Log Weight" onClose={()=>setModal(null)}>
        {(()=>{
          ...
          return <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Field label="Notes"><input value={wf.notes} onChange={e=>setWf(p=>({...p,notes:e.target.value}))}/></Field>
            <div style={{display:"flex",gap:10}}>
              <Btn v="secondary" onClick={()=>setModal(null)} full>Cancel</Btn>
              <Btn onClick={async()=>{if(!wf.weight)return;const{data}=await db.addWeight(userId,{dogId:dog.id,date:wf.date,weight:parseFloat(wf.weight),notes:wf.notes});if(data)dispatch({t:"ADD_WT",w:data});setModal(null);}} full>Save</Btn>
            </div>
          </div>;
        })()}
      </Modal>}
  ...
  if(section==="vets")return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    ...
    {vets.map(v=>(<Card key={v.id}>
      <div style={{display:"flex",justifyContent:"space-between"}}>
        ...
        {v.clinic&&<div style={{fontSize:13,color:"#5A4535"}}>{v.clinic}</div>}
        {v.phone&&<a href={`tel:${v.phone}`} style={{fontSize:13,color:"#2D7D6F",display:"flex",alignItems:"center",gap:4,marginTop:4,textDecoration:"none"}}><Ic n="phone" s={12} c="#2D7D6F"/>{v.phone}</a>}</div>
        <button onClick={async()=>{await db.deleteSavedVet(v.id);dispatch({t:"DEL_VET",id:v.id});}} style={{background:"#C4714A14",border:"1px solid #C4714A44",borderRadius:8,padding:"5px 8px",color:"#C4714A"}}><Ic n="trash" s={13}/></button>
      </div>
    </Card>))}
    {modal?.type==="addVet"&&<Modal title="Save Vet Contact" onClose={()=>setModal(null)}>
      {(()=>{
        const[vf,setVf]=useState({name:"",clinic:"",phone:"",address:"",email:""});
        return <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <Field label="Vet Name"><input value={vf.name} onChange={e=>setVf(p=>({...p,name:e.target.value}))} placeholder="Dr. Johnson"/></Field>
          <Field label="Clinic"><input value={vf.clinic} onChange={e=>setVf(p=>({...p,clinic:e.target.value}))}/></Field>
          <Field label="Phone"><input value={vf.phone} onChange={e=>setVf(p=>({...p,phone:e.target.value}))}/></Field>
          <div style={{display:"flex",gap:10}}>
            <Btn v="secondary" onClick={()=>setModal(null)} full>Cancel</Btn>
            <Btn onClick={async()=>{if(!vf.name)return;const{data}=await db.addSavedVet(userId,vf);if(data)dispatch({t:"ADD_VET",v:data});setModal(null);}} full>Save</Btn>
          </div>
        </div>;
      })()}
    </Modal>}
  ...
  if(section==="docs")return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>{backBtn}<h3 style={{fontFamily:"'Lora',serif",fontSize:20,flex:1}}>Documents</h3>
      {premium&&<Btn sm onClick={()=>setModal("addDoc")}><Ic n="plus" s={14}/> Upload</Btn>}</div>
    {premium?<>
      {docs.length===0?<Empty icon="doc" title="No documents" sub="Upload photos or scans of vet records." action={<Btn onClick={()=>setModal("addDoc")}><Ic n="camera" s={14}/> Upload</Btn>}/>
        :docs.map(d=>(<Card key={d.id}>
          <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
            <div style={{width:56,height:56,borderRadius:10,background:"#FFFFFF",border:"1px solid #E8DDD0",display:"flex",alignItems:"center",justifyContent:"center",color:"#8B7355",flexShrink:0}}><Ic n="doc" s={22} c="#8B7355"/></div>
            <div style={{flex:1}}><div style={{fontWeight:600}}>{d.name}</div>
            <div style={{fontSize:12,color:"#5A4535",marginTop:2}}>{fmt(d.doc_date)} · {DL[d.doc_type]||d.doc_type}</div></div>
            <button onClick={async()=>{await db.deleteDocument(d.id,d.file_path);dispatch({t:"DEL_DOC",id:d.id});}} style={{background:"#C4714A14",border:"1px solid #C4714A44",borderRadius:8,padding:"5px 8px",color:"#C4714A"}}><Ic n="trash" s={13}/></button>
          </div>
        </Card>))}
  ...
  if(section==="qr"){
    if(!premium)return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>{backBtn}<h3 style={{fontFamily:"'Lora',serif",fontSize:20}}>QR Health Card</h3></div>
    ...
    const payload=JSON.stringify({name:dog.name,breed:dog.breed,microchip:dog.microchip||"N/A",allergies:als||"None",medications:mds||"None",emergency:`${dog.emergency_contact||""} ${dog.emergency_phone||""}`.trim()||"N/A"});
    const qrUrl=`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(payload)}`;
    return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>{backBtn}<h3 style={{fontFamily:"'Lora',serif",fontSize:20}}>QR Health Card</h3></div>
      <Card>
        <p style={{color:"#5A4535",fontSize:13,marginBottom:16}}>Any vet can scan this to see {dog.name}'s key info.</p>
        <div style={{display:"flex",justifyContent:"center"}}><img src={qrUrl} style={{borderRadius:12,border:"2px solid #E8DDD0",background:"#fff",padding:8}}/></div>
      </Card>
    </div>);
  }

  const tiles=[
    {id:"weight",icon:"weight",label:"Weight History",desc:"Track & chart weight over time",color:"#2D7D6F",locked:!premium},
    {id:"vets",icon:"map",label:"Saved Vets",desc:"Contacts + find vets nearby",color:"#2D7D6F",locked:false},
    {id:"docs",icon:"doc",label:"Documents",desc:"Upload vet records & certificates",color:"#E8A838",locked:!premium},
    {id:"qr",icon:"qr",label:"QR Health Card",desc:"Scannable card for any vet",color:"#5A4535",locked:!premium},
  ];
  return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    <h3 style={{fontFamily:"'Lora',serif",fontSize:20}}>More</h3>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      {tiles.map(tile=>(<Card key={tile.id} onClick={()=>setSection(tile.id)} style={{padding:20,opacity:tile.locked?.75:1}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{color:tile.color,marginBottom:8}}><Ic n={tile.icon} s={24} c={tile.color}/></div>
          {tile.locked&&<Ic n="lock" s={14} c="#5A4535"/>}
        </div>
        <div style={{fontWeight:600,fontSize:15,marginBottom:4}}>{tile.label}</div>
        <div style={{fontSize:12,color:"#5A4535"}}>{tile.desc}</div>
        {tile.locked&&<div style={{fontSize:11,color:"#E8A838",marginTop:6}}>✦ Premium</div>}
      </Card>))}
    </div>
...
const DogDetail=({dog,state,dispatch,userId,tier,onBack,onUpgrade,userEmail})=>{
  ...
  return(<div style={{minHeight:"100vh",background:"#FAF6F0",paddingBottom:80}}>
    <div style={{background:"#1E5C52",borderBottom:"1px solid #1E5C52",padding:"14px 16px",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:10,maxWidth:680,margin:"0 auto"}}>
        ...
        {premium
          ?<button onClick={()=>setShowScan(true)} style={{background:"linear-gradient(135deg,#2D7D6F22,#2D7D6F22)",border:"1px solid #2D7D6F55",borderRadius:10,padding:"7px 11px",color:"#2D7D6F",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:5,cursor:"pointer"}}>
            <Ic n="camera" s={14} c="#2D7D6F"/>AI Scan
          </button>
          :<button onClick={upgrade} style={{background:"#E8A83820",border:"1px solid #E8A83844",borderRadius:10,padding:"7px 11px",color:"#E8A838",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:5,cursor:"pointer"}}>
            <Ic n="crown" s={14} c="#E8A838"/>Upgrade
          </button>}
      </div>
    </div>
    ...
    {showScan&&<AIScanModal dog={dog} userId={userId} onSave={()=>{/* reload data */}} onClose={()=>setShowScan(false)}/>}
...
const Home=({state,dispatch,userId,tier,userEmail,onSignOut})=>{
  ...
  if(selDog){
    ...
    if(dog)return <DogDetail dog={dog} state={state} dispatch={dispatch} userId={userId} tier={tier}
      userEmail={userEmail} onBack={()=>setSelDog(null)} onUpgrade={()=>setShowUpgrade(true)}/>;
  }
  ...
  return(<div style={{minHeight:"100vh",background:"#FAF6F0"}}>
    <div style={{background:"#1E5C52",borderBottom:"1px solid #1E5C52",padding:"16px 16px"}}>
      <div style={{maxWidth:680,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontFamily:"'Lora',serif",fontSize:28,letterSpacing:"-.01em",color:"#FFFFFF"}}>🐾 <span style={{color:"#FFFFFF",fontWeight:900}}>Your</span><span style={{color:"#F5C45E",fontWeight:900}}>Pet</span><span style={{color:"#FFFFFF",fontWeight:900}}>Pass</span></div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {!premium&&<button onClick={()=>setShowUpgrade(true)} style={{background:"#E8A83820",border:"1px solid #E8A83844",borderRadius:10,padding:"7px 12px",color:"#E8A838",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",gap:5,cursor:"pointer"}}>
            <Ic n="crown" s={13} c="#E8A838"/>Premium
          </button>}
    ...
    <div style={{maxWidth:680,margin:"0 auto",padding:"20px 16px"}}>
      {state.dogs.length===0?(<div style={{textAlign:"center",paddingTop:60}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h2 style={{fontFamily:"'Lora',serif",fontSize:22}}>Your Pets</h2>
          <Btn sm onClick={()=>setAddDog(true)}><Ic n="plus" s={14}/> Add Pet</Btn>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {state.dogs.map(dog=>{
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:18}}>
          {[{l:"Dogs",v:state.dogs.length,i:"paw",c:"#2D7D6F"},{l:"Vaccines",v:state.vaccinations.length,i:"syringe",c:"#2D7D6F"},{l:"Visits",v:state.visits.length,i:"stethoscope",c:"#5A4535"}].map(s=>(
            <Card key={s.l} style={{padding:14,textAlign:"center"}}>
              <div style={{color:s.c,marginBottom:6}}><Ic n={s.i} s={18} c={s.c}/></div>
              <div style={{fontFamily:"'Lora',serif",fontSize:22}}>{s.v}</div>
              <div style={{fontSize:11,color:"#5A4535"}}>{s.l}</div>
            </Card>))}
        </div>
      </>)}
    </div>
...
export default function YourPetPass({userId,profile,onSignOut}){
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      const[{data:dogs},{data:vacc},{data:meds},{data:alrg},{data:visits},{data:weights},{data:vets},{data:docs}]=await Promise.all([
        db.getDogs(userId),db.getVaccinations(userId),db.getMedications(userId),
        db.getAllergies(userId),db.getVisits(userId),db.getWeights(userId),
        db.getSavedVets(userId),db.getDocuments(userId),
      ]);
      dispatch({t:"LOAD",s:{
        dogs:dogs||[],vaccinations:vacc||[],medications:meds||[],
        allergies:alrg||[],visits:visits||[],weights:weights||[],
        vets:vets||[],documents:docs||[],
      }});
      setReady(true);
    };
    load();
    ...
    if(params.get("payment")==="success"){
      ...
      setTimeout(()=>load(),2000); // reload after short delay to get updated tier
    }
  },[userId]);

  if(!ready)return(
    <div style={{minHeight:"100vh",background:"#FAF6F0",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{fontFamily:"'Nunito',sans-serif",fontSize:28,fontWeight:900,color:"#2D7D6F"}}>🐾 <span style={{color:"#1E5C52"}}>Your</span><span style={{color:"#E8A838"}}>Pet</span><span style={{color:"#1E5C52"}}>Pass</span></div>
    </div>
  );
  ...
  return(
    <ErrorBoundary>
      <style>{GLOBAL}</style>
      <Home state={state} dispatch={dispatch} userId={userId} tier={tier}
