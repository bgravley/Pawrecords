...
const buildRecordHTML=(dog,state)=>{
...
// html2pdf renders whatever's in a DOM container via html2canvas. Setting a
// plain <div>'s innerHTML to a FULL document string (<!DOCTYPE>, <html>,
// <head>, <body>) is invalid — browsers parsing an innerHTML fragment don't
// accept those structural tags, and silently drop them along with whatever
// content the parser couldn't place, which is exactly why the emailed PDF
// link opened to a blank page. Email clients are far more lenient about
// malformed nested HTML than a browser's innerHTML parser is, which is why
// the email itself looked fine while the PDF was empty. Extracting just the
// <style> block (a <style> tag is valid flow content inside a <div> in
// HTML5) plus the actual <body> contents avoids the invalid nesting
// entirely instead of relying on the browser to guess what to keep.
const extractPdfFragment=(fullHtml)=>{
  const style=fullHtml.match(/<style>[\s\S]*?<\/style>/)?.[0]||"";
  const body=fullHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/)?.[1]||fullHtml;
  return style+body;
};
...
const exportHTML=(dog,state)=>{
  ...
  container.innerHTML=extractPdfFragment(html);
...
const EmailRecordModal=({dog,state,userEmail,onClose})=>{
  ...
  const handleSend=async()=>{
    if(!isValidEmail(recipientEmail))return setErr("Please enter a valid email address.");
    ...
    try{
      ...
      try{
        const container=document.createElement("div");
        container.innerHTML=extractPdfFragment(htmlContent);
        document.body.appendChild(container);
  ...
  return(<Modal title={`Email ${dog.name}'s Records`} onClose={onClose}>
    {sent?(
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <Btn v="secondary" onClick={onClose} full>Cancel</Btn>
          <Btn onClick={handleSend} disabled={sending||!recipientEmail} full>{sending?"Sending...":"Send Record"}</Btn>
        </div>
      </div>
    )}
  </Modal>);
...
const OverviewTab=({dog,state,userId,tier,setModal,onUpgrade,onScan,dispatch})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {ptFull&&ptColor&&(<Card style={{border:`2px solid ${ptColor}55`,background:ptColor+"0D",padding:16}}>
      ...
      <div style={{background:"#FFFFFF",borderRadius:10,padding:"10px 14px",fontSize:13,color:"#385744",lineHeight:1.6}}>{dog.pet_type==="service_animal"?"✈️ Service-animal air-travel rights and forms vary by route and jurisdiction. For flights to, within, or from the U.S., DOT rules require airlines to recognize qualifying trained service dogs, subject to safety, health, and permitted-form requirements. Check the destination government and carrier rules before each trip.":"✈️ Emotional support animals are treated differently from service animals. Airline, destination, and housing rules vary, so check the current government, carrier, and property requirements that apply to your trip."}</div>
      ...
      <div style={{background:"#164D44",borderBottom:"1px solid #0D3830",padding:"8px 16px"}}>
        <div style={{display:"flex",gap:8,maxWidth:680,margin:"0 auto"}}>
          {[
            ...
            {label:"Email",icon:"mail",action:()=>setModal("emailRecord"),premium:true},
