...
|---|---|---|---|---|---|---|
| AUTH-001 | Authorization | Anonymous/User A/User B cannot cross-access sensitive records | Critical | Finding identified | Fix before next release | Automated regression suite being added to existing smoke framework |
...
| SUP-001 | Supply chain | Third-party JS and package dependencies are inventoried and trust-reviewed | High | Finding identified | Scheduled remediation | cdnjs runtime dependencies identified; bundling/self-host/SRI review remains |
| REPO-001 | Repository exposure | Public repo contains no secrets/private customer data and history is reviewed | Critical | Finding identified | Fix before next release | Repo visibility confirmed public; history scan still required |
