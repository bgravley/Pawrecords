...
checks = {
    ...
    'browser HTML is not accepted as request data': "const { recipientEmail, petId, petName, note, pdfUrl } = req.body || {};" in text and "htmlContent" not in text,
    ...
    'legacy pet-name selector is owner scoped': "selector = `name=eq.${encodeURIComponent(petName.trim())}`" in text,
