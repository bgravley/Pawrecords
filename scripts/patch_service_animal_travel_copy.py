...
if count != 1:
    raise SystemExit(f'expected blanket service-animal airline claim exactly once, found {count}')
...
count = text.count(old_esa)
if count != 1:
    raise SystemExit(f'expected ESA travel copy exactly once, found {count}')
text = text.replace(old_esa, new_esa, 1)

p.write_text(text, encoding='utf-8')
print('service-animal and ESA travel copy tightened')
